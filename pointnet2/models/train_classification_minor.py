...
def parse_args():
    ...
    parser.add_argument('--model', default='handpointnet2_minor', help='model name [default: pointnet_cls]')
...
def test(model, loader):
    ...
    for j, (points, target) in tqdm(enumerate(loader), total=len(loader)):
        if not args.use_cpu:
            ...
            target = torch.hstack((target[:,0:3],target[:,3:6],target[:,15:18],target[:,27:30],target[:,39:42]))
...
def main(args):
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for batch_id, (points, target) in pbar:
            ...
            optimizer.step()
            global_step += 1
            pbar.set_description("Loss:%f" % current_mse)
        ...
        with torch.no_grad():
            avg_mse = test(classifier.eval(), testDataLoader)
