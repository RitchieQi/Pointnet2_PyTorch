...
from ICVL_dataset import ICVLHand_16Ver
import importlib
...
def test(model, loader):
    ...
    for j, (points, target) in tqdm(enumerate(loader), total=len(loader)):
        ...
        for i in range(32):
            ...
            for j in range(16):
                if refinePC[i][j].unique(dim=0).size()[0] < 5:
                    if j == 0:
...
def main(args):
    ...
    test_dataset = ICVLHand_16Ver(task = 'test')
    ...
    model_name = 'handpointnet2_vote_refine_msg_icvl'
    ...
    checkpoint = torch.load(str(experiment_dir) + '/checkpoints/ICVL16MSG.pth')
