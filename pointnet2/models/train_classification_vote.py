...
def parse_args():
    ...
    parser.add_argument('--model', default='handpointnet2_vote_v3', help='model name [default: pointnet_cls]')
...
def test(model, loader, func):
    ...
    for j, (points, target) in tqdm(enumerate(loader), total=len(loader)):
        ...
        #print(pred.size())
        total_loss = func(refine.float(), target.float())
        

        mse_list.append(total_loss.cpu())
...
def main(args):
    ...
    try:
        checkpoint = torch.load(str(exp_dir) + '/checkpoints/best_model180.pth')
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for batch_id, (points, target) in pbar:
            ...
            loss1 = criterion(est, target.float())
            loss2 = criterion(refine, target.float())
            loss =  10*loss2#1*loss1 + 1*loss2

            current_mse = loss.item()
            loss_est = loss1.item()
            loss_refine = loss2.item()
            mse.append(current_mse)
            loss.backward()
            optimizer.step()
            global_step += 1
            pbar.set_description("Loss:%f,Loss_est:%f,Loss_refine:%f" % (current_mse,loss_est ,loss_refine))
        ...
        with torch.no_grad():
            avg_mse = test(classifier.eval(), testDataLoader,criterion)
